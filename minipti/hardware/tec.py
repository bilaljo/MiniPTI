...
class Driver(hardware.serial_device.Driver):
    ...
    def set_pid_d_value(self, laser: str):
        tec: Tec = self[laser]
        d_value_hex = f"{tec.PID.D_parameter:0{hardware.serial_device.Driver._NUMBER_OF_HEX_BYTES}X}"
        channel = 1 if laser == "Pump Laser" else 2
        self.write(f"SD{channel}" + d_value_hex)

    def set_pid_p_value(self, laser: str):
        tec: Tec = self[laser]
        p_value_hex = f"{tec.PID.P_parameter:0{hardware.serial_device.Driver._NUMBER_OF_HEX_BYTES}X}"
        channel = 1 if laser == "Pump Laser" else 2
        self.write(f"SP{channel}" + p_value_hex)

    def set_pid_i_value(self, laser: str, i: int):
        tec: Tec = self[laser]
        i_value_hex = f"{tec.PID.I_parameter[i]:0{hardware.serial_device.Driver._NUMBER_OF_HEX_BYTES}X}"
    ...
    def set_setpoint_temperature_value(self, laser: str):
        ...
        setpoint_temperature_value_hex = f"{setpoint_temperature_value:0{hardware.serial_device.Driver._NUMBER_OF_HEX_BYTES}X}"
        channel = 1 if laser == "Pump Laser" else 2
        self.write(f"SS{channel}" + setpoint_temperature_value_hex)

    def set_loop_time_value(self, laser: str):
        tec: Tec = self[laser]
        loop_time_value_hex = f"{tec.system_parameter.loop_time:0{hardware.serial_device.Driver._NUMBER_OF_HEX_BYTES}X}"
        channel = 1 if laser == "Pump Laser" else 2
    ...
    def set_reference_resistor_value(self, laser: str):
        ...
        reference_resistor_value_hex = f"{reference_resistor_value:0{hardware.serial_device.Driver._NUMBER_OF_HEX_BYTES}X}"
        channel = 1 if laser == "Pump Laser" else 2
        self.write(f"SC{channel}" + reference_resistor_value_hex)

    def set_max_power_value(self, laser: str):
        tec: Tec = self[laser]
        max_power_value_hex = f"{tec.system_parameter.max_power:0{hardware.serial_device.Driver._NUMBER_OF_HEX_BYTES}X}"
        channel = 1 if laser == "Pump Laser" else 2
